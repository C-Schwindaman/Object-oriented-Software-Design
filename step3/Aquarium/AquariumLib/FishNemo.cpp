 ...
 */
FishNemo::FishNemo(Aquarium *aquarium) : Item(aquarium)
{
    mFishImage = make_unique<wxImage>(FishNemoImageName, wxBITMAP_TYPE_ANY);
 ...
 * @return true if hit.
 */
bool FishNemo::HitTest(int x, int y)
{
    double wid = mFishBitmap->GetWidth();
    double hit = mFishBitmap->GetHeight();
