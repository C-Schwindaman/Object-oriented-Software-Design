...
/**
 * Handle the update edit move event
 * @param event Command event object
 */
void ViewEdit::OnUpdateEditMove(wxUpdateUIEvent& event)
{
    if (mMode == Mode::Move) { event.Check(true); } else { event.Check(false); }
}

/**
 * Handle the update edit rotate event
 * @param event Command event object
 */
void ViewEdit::OnUpdateEditRotate(wxUpdateUIEvent& event)
{
    if (mMode == Mode::Rotate) { event.Check(true); } else { event.Check(false); }
}
