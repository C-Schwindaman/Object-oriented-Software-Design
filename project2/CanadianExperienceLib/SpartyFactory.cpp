...
std::shared_ptr<Actor> SpartyFactory::Create(std::wstring imagesDir)
{
    ...
    auto amFlag = std::make_shared<ImageDrawable>(L"American Flag", imagesDir +L"/amflag.png");
    amFlag->SetCenter(wxPoint(4, 250));
    amFlag->SetPosition(wxPoint(75, 105));
    larm->AddChild(amFlag);

    actor->AddDrawable(amFlag);
