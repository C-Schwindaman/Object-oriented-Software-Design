...
public:
    ...
    std::vector<std::shared_ptr<MemberReport>> GetReports()
    {
        ...
        int container = mFirstContainer;
        int location = mFirstLocation;

        while(container >= 0)
        {
            std::shared_ptr<ReportHolder> current = mContainers[container][location];
            list.push_back(current->mReport);

            container = current->mNextContainer;
            location = current->mNextLocation;
        }
...
TEST_F(CityReportTest, Iterator)
{
    ...
    // Ensure the iterator thinks it is not at the end yet
